...
- [T4] Swift cross-compile (task #4), app `autotest-swift` (swift/simple-api template, Hummingbird+OTel, port 3103):
  ...
  - Swift build retry from home dir: running in background.
- [T6-partial] 60s unfiltered `device logs` stream during HelloVLM inference: 46 JSON lines, 0 unparseable, services hellovlm_llm/hellovlm_app/wendy-agent, no gaps/disconnects. PASS.
