...
struct PlatformUSBMonitorTests {
    ...
    func testUSBMonitorLifecycle() async throws {
        ...
        let monitor = PlatformUSBMonitor(deviceDiscovery: mockDiscovery, logger: logger, pollingInterval: .milliseconds(100))
    ...
    @Test("USB monitor detects EdgeOS devices")
    func testEdgeOSDeviceDetection() async throws {
        let logger = createTestLogger()
        let mockDiscovery = MockUSBDeviceDiscovery(logger: logger)
        
        let monitor = PlatformUSBMonitor(deviceDiscovery: mockDiscovery, logger: logger, pollingInterval: .milliseconds(50))
        ...
        // Add mock EdgeOS device
        await mockDiscovery.addMockEdgeOSUSBDevice(name: "EdgeOS Device 12345")
        ...
        // Verify EdgeOS device was detected
        #expect(await mockDiscovery.findUSBDevicesCallCount >= 1)
        ...
        if let firstEvent = events.first {
            ...
            case .connected(let deviceInfo):
                ...
                #expect(deviceInfo.name == "EdgeOS Device 12345")
            case .disconnected:
    ...
    @Test("USB monitor ignores non-EdgeOS devices for events but still tracks them")
    func testNonEdgeOSDeviceHandling() async throws {
        let logger = createTestLogger()
        let mockDiscovery = MockUSBDeviceDiscovery(logger: logger)
        ...
        // Non-EdgeOS devices should not generate events (according to the current implementation)
        let events = await collector.getEvents()
        
        // The monitor tracks all devices internally but only generates events for EdgeOS devices
        // This test verifies the monitor doesn't crash with non-EdgeOS devices
    ...
    func testDeviceChangeDetection() async throws {
        ...
        // Add a new EdgeOS device to simulate connection
        await mockDiscovery.addMockEdgeOSUSBDevice(name: "New EdgeOS Device", vendorId: 0x1D6B, productId: 0x0105)
    ...
    func testMultipleDeviceHandlers() async throws {
        ...
        // Add mock EdgeOS device
        await mockDiscovery.addMockEdgeOSUSBDevice()
