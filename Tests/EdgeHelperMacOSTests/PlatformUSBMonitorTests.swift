    ...
    struct PlatformUSBMonitorTests {
        ...
        func testNonEdgeOSDeviceHandling() async throws {
            ...
            // ✅ FIXED: Verify no events were generated for non-EdgeOS devices
            let events = await collector.getEvents()
            #expect(events.isEmpty, "Non-EdgeOS devices should not generate any events")

            // ✅ OR more specific assertion:
            #expect(
                events.count == 0,
                "Expected 0 events for non-EdgeOS device, got \(events.count)"
            )
        }
