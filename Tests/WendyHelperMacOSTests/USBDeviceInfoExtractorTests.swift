    ...
    struct USBDeviceInfoExtractorTests {
        ...
        @Test("Extract Wendy device info from valid properties")
        func testExtractWendyDeviceInfo() {
            let logger = createTestLogger()
            let extractor = USBDeviceInfoExtractor(logger: logger)

            // Create test properties for Wendy device
            let properties: [String: Any] = [
                "idVendor": NSNumber(value: 0x1D6B),  // Wendy vendor ID
                "idProduct": NSNumber(value: 0x0104),  // Wendy product ID
                "USB Product Name": "Wendy Device",
            ]
        ...
        @Test("Extract non-Wendy device info")
        func testExtractNonWendyDeviceInfo() {
            let logger = createTestLogger()
            let extractor = USBDeviceInfoExtractor(logger: logger)

            // Create test properties for non-Wendy device
            let properties: [String: Any] = [
