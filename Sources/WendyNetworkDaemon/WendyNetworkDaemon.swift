    ...
    @main
    struct WendyNetworkDaemon: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "wendy-network-daemon",
            abstract: "Wendy privileged network configuration daemon",
            version: "1.0.0"
        )
        ...
        func run() async throws {
            ...
            let logger = Logger(label: "sh.wendy-network-daemon")

            logger.info("Starting Wendy Network Daemon")

            // Create and start the XPC listener - don't call .activate() to avoid API misuse
            let service = WendyNetworkDaemonService(logger: logger)

            // because this runs in a launchd service, we don't need to activate it, so no need for the variable
            _ = try XPCListener(
                service: kWendyNetworkDaemonServiceName
            ) { request in
            ...
            logger.info(
                "Wendy Network Daemon listener created",
                metadata: ["service": "\(kWendyNetworkDaemonServiceName)"]
            )
    ...
    final class WendyNetworkDaemonService: Sendable {
        ...
        func handleIncomingSessionRequest(
            ...
            return request.accept { (session: XPCSession) -> WendyNetworkDaemonHandler in
                ...
                return WendyNetworkDaemonHandler(logger: self.logger)
            }
        }
        ...
        private func validateConnection(_ pid: pid_t) -> Bool {
            ...
            let status = SecRequirementCreateWithString(
                "identifier \"com.wendyos.wendy-cli\"" as CFString,
                [],
                &requirement
            )
    ...
    @available(macOS 14.0, *)
    final class WendyNetworkDaemonHandler: XPCPeerHandler {
...
// Entry point handled automatically by ArgumentParser
    ...
    @main
    struct WendyNetworkDaemon {
        static func main() {
            print("wendy-network-daemon is only supported on macOS")
            exit(1)
        }
    }
