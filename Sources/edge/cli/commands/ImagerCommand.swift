...
struct ImagerCommand: AsyncParsableCommand {
    ...
    static let configuration = CommandConfiguration(
        ...
        subcommands: [ListCommand.self, ListDevicesCommand.self, WriteCommand.self, WriteDeviceCommand.self]
    )
    ...
    struct WriteDeviceCommand: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            let manifestManager = ManifestManagerFactory.createManifestManager()
            let (imageUrl, imageSize) = try await manifestManager.getLatestImageInfo(for: deviceName)
            
            print("📥 Found image: \(imageUrl.lastPathComponent)")
            print("   Size: \(ByteCountFormatter.string(fromByteCount: Int64(imageSize), countStyle: .file))")
            ...
            do {
                ...
                print("⚠️  Warning: Could not remove temporary image file: \(error.localizedDescription)")
            }
