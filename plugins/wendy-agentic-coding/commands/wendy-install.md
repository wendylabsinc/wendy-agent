...
Check whether `wendy --version` already works before running an installer. Use `curl -fsSL https://install.wendy.dev/cli.sh | bash` on macOS/Linux and `winget install WendyLabs.Wendy --source winget` on Windows. Verify with `wendy --version` and `wendy discover --json`. For MCP and hardware workflows, require Wendy CLI `2026.04.30-211221` or newer.
