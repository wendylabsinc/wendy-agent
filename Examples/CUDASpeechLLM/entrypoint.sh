...
if command -v nvidia-smi >/dev/null 2>&1; then
    ...
    if [ -n "$cuda_memory_mib" ]; then
        if [ "$cuda_memory_mib" -eq 0 ]; then
            # GB10 is a unified-memory SoC. Its driver exposes no dedicated
            # framebuffer, so nvidia-smi reports 0 MiB even though CUDA can
            # allocate from system memory. The /proc/meminfo checks above are
            # the authoritative capacity checks on DGX Spark.
            echo "NVIDIA dedicated framebuffer: 0 GiB (unified-memory GPU; using system memory checks)"
        else
