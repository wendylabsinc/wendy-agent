#!/usr/bin/env python3
"""DGX Spark device-microphone voice loop and static UI for Ultravox."""
...
WENDY_KNOWLEDGE_PATH = Path(
    os.environ.get(
        "WENDY_KNOWLEDGE_PATH",
        Path(__file__).with_name("wendy_common_knowledge.md"),
    )
)


def load_wendy_common_knowledge() -> str:
    try:
        knowledge = WENDY_KNOWLEDGE_PATH.read_text(encoding="utf-8").strip()
    except OSError as exc:
        raise RuntimeError(
            f"Unable to preload Wendy knowledge from {WENDY_KNOWLEDGE_PATH}: {exc}"
        ) from exc
    if not knowledge:
        raise RuntimeError(f"Wendy knowledge file is empty: {WENDY_KNOWLEDGE_PATH}")
    return knowledge


WENDY_COMMON_KNOWLEDGE = load_wendy_common_knowledge()
...
SYSTEM_PROMPT = (
    ...
    "markdown unless the user asks for it. You have preloaded background "
    "knowledge about Wendy below. Use it when relevant, but do not force Wendy "
    "into unrelated answers. Treat it as a dated offline snapshot rather than "
    "live website access, and say when a release-specific detail should be "
    "checked in the current documentation.\n\n"
    f"{WENDY_COMMON_KNOWLEDGE}"
)
...
def capture_commands() -> list[tuple[str, list[str]]]:
    commands: list[tuple[str, list[str]]] = []
    source = os.environ.get("AUDIO_SOURCE", "default")
    preferred = os.environ.get("AUDIO_BACKEND", "auto").lower()
    if preferred != "alsa" and shutil.which("pw-record"):
        target = "@DEFAULT_AUDIO_SOURCE@" if source == "default" else source
        commands.append(
            (
                "PipeWire",
                [
    ...
    if preferred != "pipewire" and shutil.which("arecord"):
        alsa_source = os.environ.get("ALSA_CAPTURE_DEVICE", source)
        commands.append(
            (
                "ALSA",
                [
...
def start_capture() -> tuple[subprocess.Popen | None, str]:
    for backend, command in capture_commands():
        try:
        ...
        if process.poll() is None:
            with STATE.lock:
                ...
                STATE.last_error = ""
            STATE.publish("source", {"backend": backend, "source": STATE.capture_source})
...
def capture_loop() -> None:
    ...
    while not STATE.stop.is_set():
        ...
        if process is None:
            process, _ = start_capture()
            if process is None:
                STATE.set_phase("error", "No PipeWire or ALSA capture source is available")
                time.sleep(2)
                continue
