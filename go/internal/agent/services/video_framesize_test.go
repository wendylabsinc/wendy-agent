...
func TestBuildGStreamerArgs_USB_PrefersMJPEG(t *testing.T) {
	...
	args, err := buildGStreamerArgs("/usr/bin/gst-launch-1.0",
		captureSource{devicePath: "/dev/video9", transport: camera.TransportUSB}, req,
		"x264enc", true, map[string]bool{"jpegdec": true})
	if err != nil {
...
func TestBuildGStreamerArgs_USB_RawWhenNoJpegdec(t *testing.T) {
	...
	args, err := buildGStreamerArgs("/usr/bin/gst-launch-1.0",
		captureSource{devicePath: "/dev/video9", transport: camera.TransportUSB}, req,
		"x264enc", true, map[string]bool{"x264enc": true})
	if err != nil {
		t.Fatalf("buildGStreamerArgs: %v", err)
	}
	joined := strings.Join(args, " ")
