...
// every Avahi version has always implemented it).
func probeAvahiAvailable(ctx context.Context, conn avahiConn) error {
	_, err := conn.Call(ctx, avahiServerPath, avahiGetVersionStringMethod)
	return err
}
...
// pools), emitting every resolved service until ctx is done. It always
// returns nil once ctx ends; every resolve worker has exited and the browser
// has been torn down (best-effort) before it does, satisfying the streaming
// backend contract (stream.go's lanBackendFn doc comment).
func avahiBrowse(ctx context.Context, conn avahiConn, serviceType string, emit func(MDNSService)) error {
	...
	reply, err := conn.Call(ctx, avahiServerPath, avahiServiceBrowserNewMethod,
