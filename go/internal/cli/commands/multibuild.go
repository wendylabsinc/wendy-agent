...
// means "don't skip it".
func planServicePushSkips(ctx context.Context, conn *grpcclient.AgentConnection, cwd, appID, deviceKey, platform string, appCfg *appconfig.AppConfig, services map[string]*appconfig.ServiceConfig, buildArgs map[string]string, extraEnv []string) (skip map[string]bool, hashes map[string]string) {
	...
	for name, svc := range services {
		...
		hash, err := computeBuildInputHash(contextDir, dockerfile, platform, buildArgs, serviceDeployEnv(appCfg, svc, extraEnv))
		if err != nil {
			continue
		}
		hashes[name] = hash
...
func runMultiServiceWithAgent(ctx context.Context, conn *grpcclient.AgentConnection, cwd string, appCfg *appconfig.AppConfig, opts runOptions) error {
	...
	skip, hashes := planServicePushSkips(ctx, conn, cwd, appCfg.AppID, deviceKey, platform, appCfg, services, buildArgs, opts.env)
	if n := len(skip); n > 0 {
	...
	createService := func(name string) error {
		...
		createReq := &agentpb.CreateContainerRequest{
			...
			Env:           serviceDeployEnv(appCfg, svc, opts.env),
		}
