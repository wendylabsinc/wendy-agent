...
- Common runtimes use Wendy-maintained `base:` channels, and every selected image still resolves to an immutable digest in a committed lockfile.
...
| --- | --- |
| `build.stagefile.lock.yaml` | Commit it. It records managed-base revisions, resolved image digests, remote-download checksums, and GPU profiles. |
...
| Field | Purpose |
| --- | --- |
| `name`, `base`, `from` | Give the stage a unique name and select either a Wendy-maintained base or an explicit image/earlier stage. Images are pinned by default. |
...
### Select a maintained base

Use `base:` for common language and runtime images. The Stagefile states the capability it needs; Wendy owns the maintained release line, distribution variant, and refresh policy:

| `base` | Intended use |
| --- | --- |
| `python` | Slim Python application runtime |
| `node` | Slim Node.js LTS runtime and package builds |
| `go` | Go toolchain stage |
| `rust` | Rust toolchain stage |
| `swift` | Swift toolchain stage |
| `swift-runtime` | Slim Swift runtime stage |
| `debian` | Minimal general-purpose runtime stage |

The generated Dockerfile still names a concrete official image, and the lockfile still pins its exact digest. Each managed channel also has a Wendy catalog revision. When Wendy advances a release line or deliberately picks up a rebuilt tag, the next build refreshes that channel once and writes a visible lockfile diff; subsequent builds reuse the new digest without a registry lookup.

Use `from:` when a stage inherits an earlier named stage or genuinely needs a specialized vendor image. `base:` and `from:` are mutually exclusive, and a managed base cannot set `pin: false`.
