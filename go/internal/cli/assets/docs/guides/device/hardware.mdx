...
WendyOS apps run in isolated containers by default. That means hardware such as cameras, microphones, speakers, Bluetooth adapters, and GPUs is not exposed to an app unless the app asks for it with an entitlement. (This applies to `wendyos` platform apps; `darwin` apps run natively on macOS without WendyOS container semantics.)
...
- [Camera Access](/docs/guides/device/hardware-camera)
- [Microphone Access](/docs/guides/device/hardware-microphone)
- [Speaker Access](/docs/guides/device/hardware-speakers)
- [Bluetooth Access](/docs/guides/device/hardware-bluetooth)
- [GPU Access](/docs/guides/device/hardware-gpu)
...
For all entitlement types, options, and `wendy.json` examples, see [App Entitlements](/docs/guides/device/entitlements).
