...
<Steps>
  <Step>
    ...
    <CliShot flow="init" step="wizard" alt="wendy init scaffolding the Swift audio project" />
  ...
  <Step>
    ...
    <CliShot flow="run" step="deploy" alt="wendy run deploying the Swift audio app" />
...
2.  **Check Logs**: Docker logs will show GStreamer errors.
    ...
    <CliShot flow="logs" step="logs" alt="wendy device logs streaming app logs from the device" />
