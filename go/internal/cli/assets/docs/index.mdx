---
title: Overview
description: The open source operating system and toolchain for Physical AI — robots, drones, and edge devices.
full: true
---

import { Database, Monitor, Box, Terminal, ArrowRight } from 'lucide-react';
import { HardwareShowcase } from '@/components/docs/hardware-showcase';
import { OverviewIntro } from '@/components/docs/overview-intro';
import { USPGrid } from '@/components/docs/usp-grid';

<OverviewIntro />

<USPGrid />

<a
  href="/docs/installation/developer-machine-setup"
  className="not-prose group my-8 flex flex-col items-center gap-2 border bg-fd-card px-6 py-10 text-center transition-colors hover:border-fd-primary/50 hover:bg-fd-accent no-underline"
>
  <span className="text-2xl font-semibold text-fd-card-foreground">Get Started!</span>
  <span className="text-fd-muted-foreground">Set up your developer machine and deploy your first app.</span>
  <span className="mt-4 inline-flex items-center gap-2 bg-fd-primary px-5 py-2.5 font-medium text-fd-primary-foreground transition-transform group-hover:translate-x-0.5">
    Developer Machine Setup
    <ArrowRight className="size-4" />
  </span>
</a>

## Supported Hardware

Wendy runs on edge devices with strong local compute. Plug one in over USB-C and start deploying.

<HardwareShowcase />

## Core Components

Wendy consists of three main pillars:

<Cards>
  <Card icon={<Database />} title="WendyOS">
    The Apache 2.0 open source Linux distribution optimized for Physical AI. Includes a mobile-like USB-C development experience, remote debugging, and atomic updates.
  </Card>

  <div className="relative">
    <span className="absolute -top-px right-0 z-10 bg-orange-500 text-white text-xs px-2 py-1 font-medium">Beta</span>
    <Card icon={<Terminal />} title="Wendy for Mac">
      Deploy native macOS apps to Apple Silicon Mac targets with [Wendy for Mac](/docs/wendy-for-mac) beta. Supports SwiftPM and Xcode projects with the same CLI workflow.
    </Card>
  </div>

  <div className="relative">
    <span className="absolute -top-px right-0 z-10 bg-orange-500 text-white text-xs px-2 py-1 font-medium">Coming Soon</span>
    <Card icon={<Monitor />} title="Wendy Cloud">
      A fleet management solution for OTA updates, crash reporting, and remote telemetry. Built for teams scaling from prototypes to thousands of devices.
    </Card>
  </div>

  <Card icon={<Box />} title="Developer Tools">
    CLI and VSCode extension for interactive breakpoints, hot reloading, and one-click deployment. Works with VSCode, Cursor, and Windsurf.
  </Card>

  <Card icon={<Terminal />} title="Wendy Frameworks">
    Performance-first Swift, Python, and Rust frameworks for running AI models, controlling I/O, and handling high-speed telemetry.
  </Card>
</Cards>
