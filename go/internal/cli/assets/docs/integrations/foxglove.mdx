...
Unitree's public `unitree_go` and `unitree_api` message definitions to the bridge
image. This lets Foxglove decode core custom topics such as `/lowstate` and
`/sportmodestate`, and makes the normal Unitree invocation simply:
...
repository (for example onboard-only `unitree_interfaces` messages) remain
unavailable unless their definitions are installed separately.
