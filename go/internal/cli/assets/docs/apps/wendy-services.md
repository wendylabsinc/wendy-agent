...
- Top-level `wendy.json.files` are synced and bind-mounted for single-container `wendy run` deployments (WendyOS/Linux and native macOS). They are not supported for multi-service `services`-map deployments yet, including `wendy run --service <name>`. File-sync semantics need to be service-scoped so Wendy can decide which files mount into which container instead of silently applying one top-level set to a single service.
