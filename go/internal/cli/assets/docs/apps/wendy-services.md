...
- Top-level `wendy.json.files` are not supported for multi-service deployments yet, including `wendy run --service <name>`. File-sync semantics need to be service-scoped so Wendy can decide which files mount into which container instead of silently applying one top-level set to a single service.
- Top-level `wendy.json.files` are not supported for multi-service deployments yet, including `wendy run --service <name>`. File-sync semantics need to be service-scoped so Wendy can decide which files mount into which container instead of silently applying one top-level set to a single service.
